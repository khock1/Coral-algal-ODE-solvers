...
for i = 1:length(parameter_range)
    z = parameter_range(i);%substitute z here to perform stability analysis with a different paramter
    ...
    for jj = 1:length(ss1.C)%sequentially plot equilibria for a given range of parameter values
        if isreal(ss1.C(jj)) && isreal(ss1.M(jj))%only concerned with real equilibria
            if all([ss1.C(jj) ss1.M(jj)]>=0 & [ss1.C(jj) ss1.M(jj)]<=1)%only concerned with nontrivial solutions, i.e. if proportion of coral and macroalgal cover is less than 1
                jacb_eigv = eig(double(subs(J,[C M],[ss1.C(jj) ss1.M(jj)])));%substitute equilibria into Jacobian to obtain eigenvalues
                if any(jacb_eigv>=0)%Jacobian eigenvalues > 0 mean this equlilibrium is a saddle
